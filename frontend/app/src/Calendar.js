...
class Calendar extends Component {
  ...
  handleCancelEvent = () => {
    // Ensure that an event is selected to be canceled
    if (this.state.eventIdToEdit) {
      axios.delete(`backendurl/events/${this.state.eventIdToEdit}`)
        .then(response => {
          ...
          const updatedEvents = this.state.events.filter(event => event.id !== this.state.eventIdToEdit);
          this.setState({
            events: updatedEvents,
            showEventModal: false,
            eventIdToEdit: null,
      ...
      console.error("No event selected to cancel");
    }
  };
